...
public class Variants {
    ...
    public static void validateEmptyVariants() {
        variants.keySet().forEach(Variants::resetVariants);
    }

    public static void applyBlacklists() {
        for (EntityType<?> mob : variants.keySet()) {
            //TODO: Check this.
            if (mob.equals(EntityType.CAT) /*|| mob.equals(Mob.NULL)*/ || variants.get(mob) == null) {
                continue;
            }
    ...
    //TODO: Change this to Identifier? And make the code!
    public static EntityType<?> getMob(String mobId) {
        final EntityType<?>[] toReturn = {null};
        variants.forEach((entityType, variants) -> {
            if(toReturn[0] != null) return;
            for (MobVariant variant : variants) {
                if (variant.getIdentifier().equals(Identifier.tryParse(mobId))) {
                    toReturn[0] = entityType;
                    return;
                }
            }
        });
        if(toReturn[0] != null) return toReturn[0];

        throw new IllegalArgumentException("Unknown mob: " + mobId);
    }
